...
//******************************************************OBTENCIÓN DE ARCHIVOS EMBEBIDOS*********************************************************
...
//******************************************************SOLICITUDES Y ENVÍOS DEL SERVIDOR*********************************************************
...
/**
 * Sends the dashboard.html page.
 * @param req HTTP request for which the uri needs to be handled.
 * @return ESP_OK
 */
static esp_err_t http_server_dashboard_html_handler(httpd_req_t *req)
{
	
	HTTP_DEBUG("dashboard.html requested");
...
/**
 * dashboard.js get handler is requested when accessing the web page.
 * @param req HTTP request for which the uri needs to be handled.
 * @return ESP_OK
 */
static esp_err_t http_server_dashboard_js_handler(httpd_req_t *req)
{
	HTTP_DEBUG("dashboard.js requested");
...
static esp_err_t form_post_dashboard_handler(httpd_req_t *req) {

	HTTP_DEBUG("form_post_dashboard_handler requested");
    ...
    cJSON *red_val = cJSON_GetObjectItem(json, "red_str");
    cJSON *green_val = cJSON_GetObjectItem(json, "green_str");
	cJSON *blue_val = cJSON_GetObjectItem(json, "blue_str");


    if (threshold && red_val && green_val && blue_val) 		//Verifica si se tienen los 4 datos
    {
        // Aquí puedes utilizar los valores extraídos del JSON según sea necesario
        // ...

		HTTP_DEBUG("threshold: %s, red_val: %s, green_val: %s, blue_val: %s\r\n",threshold->valuestring,red_val->valuestring,green_val->valuestring,blue_val->valuestring);


		float power = potencia_get();

		rgb_led_pwm_init();
		if(power >= atoi(threshold->valuestring))
		{
			printf("LLEGUE A ESTE PUNTO\r\n");
			rgb_led_set_color(atoi(red_val->valuestring), atoi(green_val->valuestring), atoi(blue_val->valuestring)); //Mandar los valores al LED
...
static esp_err_t http_server_get_dht_sensor_readings_json_handler(httpd_req_t *req){
	...
	float prom_temp = tension_get();
	float prom_current = current_get();

	sprintf(dhtSensorJSON, "{\"temp\":\"%.1f\",\"humidity\":\"%.1f\"}", prom_temp, prom_current);
...
static httpd_handle_t http_server_configure(void)
{
	...
	if (httpd_start(&http_server_handle, &config) == ESP_OK)
	{
		...
		// register dashboard.html handler
		httpd_uri_t dashboard_html = {
				.uri = "/dashboard",
				.method = HTTP_GET,
				.handler = http_server_dashboard_html_handler,
				.user_ctx = NULL
		};
		httpd_register_uri_handler(http_server_handle, &dashboard_html);
		...
		// register dashboard.js handler
		httpd_uri_t dashboard_js = {
				.uri = "/dashboard.js",
				.method = HTTP_GET,
				.handler = http_server_dashboard_js_handler,
				.user_ctx = NULL
		};
		httpd_register_uri_handler(http_server_handle, &dashboard_js);
		...
		// register wifiConnectInfo.json handler
		httpd_uri_t http_dashboard_handler = {
				.uri = "/dashboard-post",
				.method = HTTP_POST,
				.handler = form_post_dashboard_handler,
				.user_ctx = server_data
		};
		httpd_register_uri_handler(http_server_handle, &http_dashboard_handler);
